...
// Buat kode_menu otomatis (hanya saat form GET)
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    $lastKode = $menu->getLastKodeMenu();
    if ($lastKode) {
        $angka = (int)substr($lastKode, 2); // Ambil angka setelah prefix KM
        $angkaBaru = $angka + 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $menu->status = $_POST['status'] ?? 'available';
    // Tambahkan baris ini:
...
<body>
        ...
        <form method="POST">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="kode_menu" name="kode_menu" 
                    value="<?php echo htmlspecialchars($kode_menu_terbaru); ?>" readonly required>
            </div>
            <div class="mb-3">
    ...
    <input type="number" name="stok" id="stok" class="form-control" min="0" required value="<?= isset($stok) ? (int)$stok : 0; ?>">
</div>
            <div class="mb-3">
